...
#if defined(NDEBUG)
# error "zcoinclassic cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x9525d1f57c600dbbe42c8e8d21d30362dee684c1969a68d0544489b848f98e5f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 8); // zcoinclassic: starting difficulty is 1 / 2^12
CBlockIndex* pindexGenesisBlock = NULL;
int nBestHeight = -1;
uint256 nBestChainWork = 0;
uint256 nBestInvalidWork = 0;
uint256 hashBestChain = 0;
uint32_t nVertcoinChainStartTime = 1487750510;
int64 nStartRewardTime = 1487750510; 
...
int64 CTransaction::nMinTxFee = 1000000; // 0.01 zcoinclassic
/** Fees smaller than this (in ztoshi) are considered zero fee (for relaying) */
int64 CTransaction::nMinRelayTxFee = 1000000; // 0.01 zcoinclassic
...
const string strMessageMagic = "zcoinclassic Signed Message:\n";
...
{
    // zcoinclassic: IsDust() detection disabled, allows any valid dust to be relayed.
    ...
    if (IsCoinBase())
    {
        ...
        if ((nHeight > 0) && (nHeight < 210000)) {
            ...
            if(!fTestNet && (GetAdjustedTime() > nStartRewardTime)){
                FOUNDER_1_SCRIPT.SetDestination(CBitcoinAddress("ZQeVnPe6vw9KruZjcCDRdHv3katjQVqjE2").Get());
                if(nHeight < 14000){
                   FOUNDER_2_SCRIPT.SetDestination(CBitcoinAddress("ZLeLSjf3edDGqH6MTq8j9MvsJK7qC3y7HF").Get());
                }else{
                   FOUNDER_2_SCRIPT.SetDestination(CBitcoinAddress("ZEsSt7RanaXcUUULMiSwfb9kCgopqcaUoK").Get());                   
                }
                FOUNDER_3_SCRIPT.SetDestination(CBitcoinAddress("ZTRsz9LjbS5x3YoaEupHSyFgxGicMcR3Vo").Get());
                FOUNDER_4_SCRIPT.SetDestination(CBitcoinAddress("ZGxymDssksQX9BMtRde7xH5RF7uP3AjuHU").Get());
                FOUNDER_5_SCRIPT.SetDestination(CBitcoinAddress("ZTG55GbGZfz6s1ecqw9TaQ9nAXVG5XETmy").Get());
...
*/
    ...
    if (!fTestNet && pindexLast->nHeight + 1 == 0) {
        CBigNum bnNew;
...
bool CBlock::CheckBlock(CValidationState &state, int nHeight, bool fCheckPOW, bool fCheckMerkleRoot, bool isVerifyDB) const
{
    ...
    // zcoinclassic: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // zcoinclassic: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x9525d1f57c600dbbe42c8e8d21d30362dee684c1969a68d0544489b848f98e5f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "zcoinclassic for everyone";
        CTransaction txNew;
        ...
        if(fTestNet)
            genMerkleRoot.SetHex("0x024f973aad651b1d74367a2a4a4968e1a36767dae7759697de212b11bf6d796b");
        else
            genMerkleRoot.SetHex("0x024f973aad651b1d74367a2a4a4968e1a36767dae7759697de212b11bf6d796b");
...
unsigned char pchMessageStart[4] = { 0xe3, 0xd9, 0xfe, 0xf2 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("zcoinclassicMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("zcoinclassicMiner : ProcessBlock, block not accepted");
    }
...
void static zcoinclassicMiner(CWallet *pwallet)
{
    printf("zcoinclassicMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("zcoinclassic-Miner");
    ...
    try { loop {
        ...
        printf("Running zcoinclassicMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
*/
    ...
    {
        printf("zcoinclassicMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&zcoinclassicMiner, pwallet));
